...
__all__ = ["ResNeXt", "ResNeXt50_64x4d", "ResNeXt101_64x4d", "ResNeXt152_64x4d",  "ADV_paper_ResNeXt50_32x4d", "ResNeXt101_32x4d",
           "ResNeXt152_32x4d"]
...
class ResNeXt():
    ...
    def net(self, input, class_dim=1000):
        ...
        conv = self.conv_bn_layer(
            ...
            stride=2,
            act='relu',
            name="adv_paper_res_conv1")  # debug
        ...
        for block in range(len(depth)):
            for i in range(depth[block]):
                if layers in [101, 152] and block == 2:
                    if i == 0:
                        conv_name = "adv_paper_res" + str(block + 2) + "a"
                    else:
                        conv_name = "adv_paper_res" + str(block + 2) + "b" + str(i)
                else:
                    conv_name = "adv_paper_res" + str(block + 2) + chr(97 + i)
        ...
        out = fluid.layers.fc(input=pool,
                              ...
                              param_attr=fluid.param_attr.ParamAttr(
                                  initializer=fluid.initializer.Uniform(-stdv, stdv), name='adv_paper_fc_weights'),
                              bias_attr=fluid.param_attr.ParamAttr(name='adv_paper_fc_offset'))
        return out
