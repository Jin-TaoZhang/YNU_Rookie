...
__all__ = [
    "DenseNet", "DenseNet121", "adv_DenseNet161", "DenseNet169", "DenseNet201",
    "DenseNet264"
]
...
class DenseNet():
    ...
    def net(self, input, bn_size=4, dropout=0, class_dim=1000):
        ...
        conv = fluid.layers.conv2d(
            ...
            padding=3,
            act=None,
            param_attr=ParamAttr(name="adv_conv1_weights"),
            bias_attr=False)
        conv = fluid.layers.batch_norm(
            input=conv,
            act='relu',
            param_attr=ParamAttr(name='adv_conv1_bn_scale'),
            bias_attr=ParamAttr(name='adv_conv1_bn_offset'),
            moving_mean_name='adv_conv1_bn_mean',
            moving_variance_name='adv_conv1_bn_variance')
        ...
        for i, num_layers in enumerate(block_config):
            conv = self.make_dense_block(
                ...
                growth_rate,
                dropout,
                name='adv_conv' + str(i + 2))
            num_features = num_features + num_layers * growth_rate
            if i != len(block_config) - 1:
                conv = self.make_transition(
                    conv, num_features // 2, name='adv_conv' + str(i + 2) + '_blk')
                num_features = num_features // 2
        conv = fluid.layers.batch_norm(
            input=conv,
            act='relu',
            param_attr=ParamAttr(name='adv_conv5_blk_bn_scale'),
            bias_attr=ParamAttr(name='adv_conv5_blk_bn_offset'),
            moving_mean_name='adv_conv5_blk_bn_mean',
            moving_variance_name='adv_conv5_blk_bn_variance')
        conv = fluid.layers.pool2d(
        ...
        out = fluid.layers.fc(
            ...
            param_attr=fluid.param_attr.ParamAttr(
                ...
                name="adv_densenet_fc_weights"),
            bias_attr=ParamAttr(name='adv_densenet_fc_offset'))
        return out
