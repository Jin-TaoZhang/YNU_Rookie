...
__all__ = ["DPN", "DPN68", "DPN92", "DPN98", "DPN107", "adv_paper_DPN131"]
...
class DPN(object):
    ...
    def net(self, input, class_dim=1000):
        ...
        conv1_x_1 = fluid.layers.conv2d(
            ...
            bias_attr=False,
            name="dpn_conv1",
            param_attr=ParamAttr(name="adv_paper_dpn_conv1_weights"), )

        conv1_x_1 = fluid.layers.batch_norm(
            input=conv1_x_1,
            act='relu',
            is_test=False,
            name="adv_paper_dpn_conv1_bn",
            param_attr=ParamAttr(name='adv_paper_dpn_conv1_bn_scale'),
            bias_attr=ParamAttr('adv_paper_dpn_conv1_bn_offset'),
            moving_mean_name='adv_paper_dpn_conv1_bn_mean',
            moving_variance_name='adv_paper_dpn_conv1_bn_variance', )
        ...
        for gc in range(4):
            ...
            convX_x_x = self.dual_path_factory(
                convX_x_x, R, R, bw, inc, G, _type1, name="adv_paper_dpn_dpn" + str(match))
            for i_ly in range(2, k_sec[gc] + 1):
                num += 1
                if num in match_list:
                    num += 1
                convX_x_x = self.dual_path_factory(
                    convX_x_x, R, R, bw, inc, G, _type2, name="adv_paper_dpn_dpn" + str(num))
        ...
        conv5_x_x = fluid.layers.batch_norm(
            ...
            act='relu',
            is_test=False,
            name="dpn_final_concat_bn",
            param_attr=ParamAttr(name='adv_paper_dpn_final_concat_bn_scale'),
            bias_attr=ParamAttr('adv_paper_dpn_final_concat_bn_offset'),
            moving_mean_name='adv_paper_dpn_final_concat_bn_mean',
            moving_variance_name='adv_paper_dpn_final_concat_bn_variance', )
        ...
        fc6 = fluid.layers.fc(input=pool5,
                              size=class_dim,
                              param_attr=ParamAttr(initializer=fluid.initializer.Uniform(-stdv, stdv), name='adv_paper_dpn_fc_weights'),
                              bias_attr=ParamAttr(name='adv_paper_dpn_fc_offset'))
