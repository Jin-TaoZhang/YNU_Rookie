...
import math
__all__ = ["adv_paper_DarkNet53"]


class adv_paper_DarkNet53():
    def __init__(self):
    ...
    def net(self, input, class_dim=1000):
        ...
        conv1 = self.conv_bn_layer(
            ...
            stride=1,
            padding=1,
            name="adv_paper_yolo_input")
        conv = self.downsample(
            conv1, ch_out=conv1.shape[1] * 2, name="adv_paper_yolo_input.downsample")

        for i, stage in enumerate(stages):
            conv = self.layer_warp(
                block_func, conv, 32 * (2**i), stage, name="adv_paper_stage.{}".format(i))
            if i < len(stages) - 1:  # do not downsaple in the last stage
                conv = self.downsample(
                    conv,
                    ch_out=conv.shape[1] * 2,
                    name="adv_paper_stage.{}.downsample".format(i))
        ...
        out = fluid.layers.fc(
            ...
            param_attr=ParamAttr(
                ...
                name='adv_paper_darknet_fc_weights'),
            bias_attr=ParamAttr(name='adv_paper_darknet_fc_offset'))
        return out
